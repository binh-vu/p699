...
from wavenet_tf.ops import causal_conv, mu_law_encode


def create_variable(name, shape):
    '''Create a convolution filter variable with the specified name and shape,
    and initialize it using Xavier initialition.'''
    # disable warning just for tf.contrib 1.15
...
def create_bias_variable(name, shape):
    ...
    return tf.Variable(initializer(shape=shape, dtype=tf.float32), name)
...
class WaveNetModel(object):
    ...
    def loss(self,
        ...
        @mynote: when global_condition_batch is None, they don't do global condition
        '''
        with tf.name_scope(name):
